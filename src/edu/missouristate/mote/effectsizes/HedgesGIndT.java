...
/**
 * Hedges's g, independent t test.
 */
public final class HedgesGIndT extends AbstractNonCentralTest {
    ...
    // DERIVED FIELDS
    // *************************************************************************
    private transient double correction;
    ...
    // *************************************************************************
    /**
     * Initialize a new instance of a HedgesGIndTTest.
     */
    public HedgesGIndT() {
        super();
        reset();
    ...
    /**
     * Return Hedge's correct factor to Cohen's d.
     *
     * @param size1 group 1 size
     * @param size2 group 2 size
     * @return correction factor
     */
    private static double calcCorrection(final double size1,
            final double size2) {
        double result;
        if (size1 + size2 > 2.25) {
            result = 1 - 3 / (4 * (size1 + size2) - 9);
        } else {
            result = 1;
        }
        return result;
     ...
     * @return g value
     */
    private static double calcG(final double mean1, final double mean2,
            final double stdDevPooled, final double correction) {
        double result;
        if (stdDevPooled != 0) {
            result = correction * (mean1 - mean2) / stdDevPooled;
        } else {
            result = 0.0;
        }
        return result;
    ...
    private void calculate() {
        ...
        if (useInputs) {
            ...
            measure = calcG(mean1, mean2, stdDevPooled, correction);
        } else {
            measure = correction * testStatistic * Math.sqrt(size
                    / (size1 * size2) * size / df);
        }
    ...
    public void setSize1(final double value) {
        ...
        stdDevPooled = calcSDPooled(stdDev1, stdDev2, size1, size2);
        stdErrPooled = calcSEPooled(stdDevPooled, size1, size2);
        correction = calcCorrection(size1, size2);
        calculate();
    }
    ...
    @CategoryAnnotation(value = Constants.DERIVED_CATEGORY)
    @NameAnnotation(value = "Hedges's Correction")
    @IndexAnnotation(value = 15)
    @DescriptionAnnotation(value = "Hedges's g is approximately equal to "
            + "Cohen's d times this multiplier which is dependent on the "
            + "size of the samples.")
    ...
    @NameAnnotation(value = "Lower g")
    @IndexAnnotation(value = 16)
    @DescriptionAnnotation(value = "Minimum value of Hedges's g on the "
            + "confidence interval.")
    @Override
    ...
    @CategoryAnnotation(value = Constants.OUTPUT_CATEGORY)
    @NameAnnotation(value = "Hedges's g")
    @IndexAnnotation(value = 17)
    @DescriptionAnnotation(value = "Hedges's g.")
    @Override
    ...
    @CategoryAnnotation(value = Constants.OUTPUT_CATEGORY)
    @NameAnnotation(value = "Upper g")
    @IndexAnnotation(value = 18)
    @DescriptionAnnotation(value = "Maximum value of Hedges's g on the "
            + "confidence interval.")
    @Override
    ...
    @DescriptionAnnotation(value = "Non-centrality parameter ("
            ...
            + "Hedges's g on the confidence interval.")
    @Override
