...
/**
 * Partial omega-squared, F test effects unbiased.
 */
public final class POmega2FEff extends AbstractNonCentralTest {
    ...
    // *************************************************************************
    /**
     * Initialize a new instance of an POmega2FEff.
     */
    public POmega2FEff() {
        super();
        reset();
     ...
     * @return d value
     */
    private static double calcOmegaSquared(final double ssEffect,
            final double ssTotal, final double dfEffect, final double mse) {
        if (ssTotal + mse != 0) {
            return (ssEffect - dfEffect * mse) / (ssTotal + mse);
        } else {
            return 0;
        }
    }
    ...
    private void calculate() {
        ...
        if (useInputs) {
            ...
            measure = calcOmegaSquared(ssEffect, ssTotal, dfEffect, getMse());            
        } else {
            measure = (dfEffect * (testStatistic - 1))
                    / ((dfEffect * (testStatistic - 1)) + getDfTotal());
        }
    ...
    public String getMeasureName() {
        return "Partial " + Constants.OMEGA_LOWER + Constants.SUPERSCRIPT2;
    }

    @Override
    public String getMeasureSymbol() {
        return Constants.OMEGA_LOWER + Constants.SUPERSCRIPT2 + "p";
    }

    @Override
    public String getTestName() {
        return "F Test Effects Unbiased";
    }
    ...
    @NameAnnotation(value = "Lower partial " + Constants.OMEGA_LOWER
            + Constants.SUPERSCRIPT2)
    @IndexAnnotation(value = 11)
    @DescriptionAnnotation(value = "Minimum value of partial " + Constants.OMEGA_LOWER
    ...
    @CategoryAnnotation(value = Constants.OUTPUT_CATEGORY)
    @NameAnnotation(value = "Partial " + Constants.OMEGA_LOWER + Constants.SUPERSCRIPT2)
    @IndexAnnotation(value = 12)
    @DescriptionAnnotation(value = "Partial " + Constants.OMEGA_LOWER + Constants.SUPERSCRIPT2
            + ".")
    @Override
    ...
    @CategoryAnnotation(value = Constants.OUTPUT_CATEGORY)
    @NameAnnotation(value = "Upper partial " + Constants.OMEGA_LOWER
            + Constants.SUPERSCRIPT2)
    @IndexAnnotation(value = 13)
    @DescriptionAnnotation(value = "Maximum value of partial " + Constants.OMEGA_LOWER
            + Constants.SUPERSCRIPT2 + " on the confidence interval.")
    @Override
